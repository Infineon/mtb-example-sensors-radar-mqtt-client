...
static void mqtt_disconnect_callback(void *pCallbackContext, IotMqttCallbackParam_t *pCallbackParam);
...
void mqtt_client_task(void *pvParameters)
{
    ...
    if (pdPASS != xTaskCreate(subscriber_task,
                              "Subscriber task",
    ...
    if (pdPASS != xTaskCreate(publisher_task,
                              "Publisher task",
    ...
    if (pdPASS !=
        xTaskCreate(radar_task, RADAR_TASK_NAME, RADAR_TASK_STACK_SIZE, NULL, RADAR_TASK_PRIORITY, &radar_task_handle))
    {
 ...
 */
    ...
    CHECK_RESULT(result,
                 ...
                 "MQTT connection failed with error '%s'!\n\n",
